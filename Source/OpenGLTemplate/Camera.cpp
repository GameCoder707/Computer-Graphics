...
CCamera::CCamera()
{
	...
	cameraSwapDelay = 0.2f;
	cameraSwapTime = cameraSwapDelay;
...
{  
	...
	angle_y = (float) (middle_x - mouse.x) / 5000.0f;
	angle_z = (float) (middle_y - mouse.y) / 5000.0f;
...
void CCamera::TrackUpdate(glm::vec3 T, glm::vec3 N, glm::vec3 B, glm::vec3 point, glm::vec3 player_pos, float dt)
{
	glm::vec3 cameraPos;
	glm::vec3 y = glm::vec3(0.0f, 1.0f, 0.0f);

	// Pressing TAB to switch camera view
	if (GetKeyState(VK_TAB) & 0x80)
	{
		if (cameraSwapTime >= cameraSwapDelay)
		{
			currentCameraMode += 1;
	...
	if (cameraSwapTime < cameraSwapDelay)
	{
		cameraSwapTime += 0.01f;
	}

	switch (CameraMode(currentCameraMode))
	{
	case ThirdPerson:
		// Set the camera to player's position first
		cameraPos = player_pos;
		
		// Then push it up by 10 units
		cameraPos += (10.0f * B);

		// And then push it back by 20 units
		cameraPos -= (20.0f * T);

		// Finally, make the camera point at a point
		// 10 units forward from the player's position
		Set(cameraPos, player_pos + 10.0f * T, y);
	...
	case FirstPerson:
		...
		// Now we push it 10 units forward and 
		// upward by 1 unit
		cameraPos += (10.0f * T);
		cameraPos += (1.0f * B);

		// And make it look at 18 units from the
		// player's position
		Set(cameraPos, player_pos + 18.0f * T, y);

		break;

	case TopDown:
		// Now the initial position is
		// set at the player
		cameraPos = player_pos;

		// Now we push 70 units upward
		// and 12 units backward
		cameraPos += (70.0f * B);
		cameraPos -= (12.0f * T);

		// And look at a point 30 units forward
		// from the player's position
		Set(cameraPos, player_pos + 30.0f * T, y);
	...
	case Side:
		...
		Set(cameraPos, player_pos + 15.0f * T, y);
...
bool CCamera::IsCameraInFreeMode()
{
